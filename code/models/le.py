...
class LaplacianEigenmaps(models.Neighbourhood):
    ...
    heat_kernel : bool, optional (default=False)
        If True, use a heat kernel for weighting edges. Otherwise, use binary adjacency.
    sigma : float, optional (default=1.0)
        Width parameter for the heat kernel if heat_kernel=True.
    """
    def __init__(self, model_args: dict, n_neighbors: int, n_components: int, heat_kernel: bool = False, sigma: float = 1.0):
    ...
    def _neigh_matrix(self, X: np.ndarray):
        ...
        neigh_matrix = neigh_matrix + neigh_matrix.T # Make symmetric
        return neigh_matrix
    ...
    def _transform(self):
        ...
        stamp_print(f"*\t {self.model_args['model']}\t transform (Laplacian)")
